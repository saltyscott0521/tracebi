...
| `TRACEBI_DISCOVERY_INTERVAL` | Seconds between live-discovery scans. A running server picks up report packages, specs and model files added to (or removed from) `reports/` and `models/` without a restart. Default `5`; `0` turns it off, so new reports need a restart. Python report modules are still found only at startup. |
