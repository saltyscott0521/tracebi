...
Copy a report folder into the project's `reports/`, a model file into
`models/`, or a pipeline file into `pipelines/`. The running server picks it up within a few seconds, with no
