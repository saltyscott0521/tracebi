...
- **Interactivity subsets, never computes.** The premium objects —
  ...
  `having`, `order_by`, and `limit` stay on the binding. The control
  on the page is that dropdown. A new grain is a new binding in
  `report.json`, then a rebuild. A number on the page is a value
  figure beside its fingerprint. The receipt downloads the sliced view and names the selection in the filename and
