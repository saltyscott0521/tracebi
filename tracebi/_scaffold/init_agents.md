...
- Interactivity: `data-tb-filter` dropdowns + `data-tb-search` inputs subset
  ...
  included. The browser displays that result. The control on the page
  is that dropdown. A new grain is a new binding in `report.json`,
  then a rebuild. A number on the page is a value figure beside its
